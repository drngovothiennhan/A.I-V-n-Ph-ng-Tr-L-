const VERSION='2.2.3-credentials-wizard-canary';
...
const DRIVE_TOKEN_SESSION_KEY='ai-office-drive-token-v223';
...
async function verifyDriveCanary(){
  try{
    if(window.AIOfficeMultiSourceV26?.verifyCanary){
      return await window.AIOfficeMultiSourceV26.verifyCanary();
    }
    const r=await fetch('/api/drive-brain',{
      method:'POST',headers:{'content-type':'application/json'},cache:'no-store',
      body:JSON.stringify({action:'search',query:DRIVE_CANARY,scopes:['02_APPROVED'],limit:5})
    });
    ...
    return{pass:Boolean(match),configured:true,fileId:match?.provenance?.fileId||null,approvalState:match?.approvalState||null};
...
function injectUi(){
  ...
  if(!document.getElementById('cred22')){
    const modal=document.createElement('div');modal.id='cred22';modal.innerHTML=`<div class="panel" role="dialog" aria-modal="true" aria-labelledby="cred22Title">
      <div class="top"><div><h3 id="cred22Title">Thiết lập kết nối A.I Văn phòng</h3><p id="cred22Summary">Đang kiểm tra cấu hình và knowledge thật…</p></div><button class="close" id="cred22Close" aria-label="Đóng">×</button></div>
      <div class="warn"><b>Không đưa API key/token vào GitHub hoặc mã frontend.</b> Wizard chỉ tạo token Drive trong phiên trình duyệt để bạn sao chép sang Apps Script + Vercel. Trạng thái Drive chỉ chuyển xanh khi đọc đúng canary thật trong 02_APPROVED.</div>
      <div class="rows" id="cred22Rows"></div>
      <div class="cred22steps" id="driveWizard"><h4>Google Drive Brain · hoàn tất kết nối an toàn</h4>
        <ol><li>Bấm <b>Sao chép Bridge code</b>.</li><li>Bấm <b>Tạo Apps Script</b>, xóa code mặc định và dán Bridge code.</li><li>Project Settings → Script Properties → thêm <b>DRIVE_BRAIN_TOKEN</b> bằng token được tạo bên dưới.</li><li>Deploy → New deployment → Web app → Execute as: Me → cấp quyền → copy URL kết thúc bằng <b>/exec</b>.</li><li>Mở Vercel Credentials và thêm <b>DRIVE_BRAIN_BRIDGE_URL</b> = URL /exec, <b>DRIVE_BRAIN_TOKEN</b> = đúng token bên dưới; áp dụng Production + Preview.</li><li>Sau khi Vercel redeploy, quay lại và bấm <b>Kiểm tra lại</b>. Hệ thống phải đọc đúng canary Approved; chỉ có biến môi trường là chưa đủ.</li></ol>
      ...
      <div class="cred22steps"><h4>XiaoZhi Voice · tùy chọn nâng cao</h4><p>Voice browser của A.I Văn phòng vẫn hoạt động khi chưa có XiaoZhi upstream. Gateway và upstream được kiểm tra riêng, không báo xanh giả chỉ vì WebSocket mở được.</p><div class="buttons"><a class="cred22btn secondary" href="${XIAOZHI_SERVER_URL}" target="_blank" rel="noopener noreferrer">Xem XiaoZhi Server ↗</a></div></div>
      <div id="cred22MissingWrap"><p><b>Phần chưa đạt</b></p><div class="secretList" id="cred22Missing"></div></div>
      <div class="buttons"><button class="cred22btn ok" id="cred22Check">Kiểm tra lại + Canary</button><a class="cred22btn primary" id="cred22Vercel" href="${VERCEL_ENV_URL}" target="_blank" rel="noopener noreferrer">Mở Vercel Credentials ↗</a></div>
    </div>`;
...
async function refreshPopup(){
  ...
  const canary=s.drive?await verifyDriveCanary():{pass:false,configured:false,reason:'DRIVE_RUNTIME_NOT_CONFIGURED'};
  const driveVerified=Boolean(s.drive&&canary?.pass);
  modal.querySelector('#cred22Rows').innerHTML=[
    ...
    row('Drive Approved knowledge canary',`${DRIVE_CANARY} · 02_APPROVED`,driveVerified?'ok':'miss'),
    row('XiaoZhi voice gateway','Render WebSocket gateway',s.xiaozhi?'ok':'opt'),
    row('XiaoZhi authentication','Trusted origin / token',s.xiaozhi?(s.xiaozhiAuth?'ok':'miss'):'opt'),
    row('Google Workspace actions','GOOGLE_WORKSPACE_ENABLED=true',s.workspace?'ok':'opt')
  ].join('');

  const blockers=[...s.blocking];
  if(s.drive&&!driveVerified)blockers.push(`DRIVE_CANARY_VERIFICATION: ${canary?.reason||'Approved canary not found'}`);
  if(!s.gemini)modal.querySelector('#cred22Summary').textContent='Gemini chưa được cấu hình.';
  else if(!s.drive)modal.querySelector('#cred22Summary').textContent='Gemini sẵn sàng. Drive canonical tồn tại nhưng Vercel → Apps Script Bridge chưa được cấp runtime.';
  else if(!driveVerified)modal.querySelector('#cred22Summary').textContent='Biến Drive đã có nhưng truy xuất knowledge Approved chưa PASS. Không bật trạng thái Drive xanh.';
  else modal.querySelector('#cred22Summary').textContent='Gemini + Drive Brain + Approved canary đã PASS. Multi-source Orchestrator có thể dùng Drive + nguồn ngoài và không phụ thuộc upload local.';

  const miss=modal.querySelector('#cred22Missing');miss.textContent=blockers.length?blockers.join('\n'):'Không còn blocker bắt buộc. Approved knowledge retrieval đã được xác minh.';
  return{...s,driveVerified,canary};
}
...
injectUi();
window.AIOfficeCredentialsV22={version:VERSION,open:openPopup,refresh:refreshPopup,close:closePopup,getDriveToken,verifyDriveCanary};
const params=new URLSearchParams(location.search);if(params.get('credentials')==='1'||!sessionStorage.getItem('ai-office-credentials-seen-v223')){sessionStorage.setItem('ai-office-credentials-seen-v223','1');setTimeout(()=>openPopup(),450)}
