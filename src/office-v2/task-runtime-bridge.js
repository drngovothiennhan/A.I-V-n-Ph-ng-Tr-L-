...
export const OFFICE_V2_TASK_BRIDGE_VERSION='2.8.0-legacy-task-sync';
...
function operationalMetadata(task,fingerprint){
  const canonical=task?.intentV32||{};
  const interaction=task?.intentV22||{};
  return {
    bridge:'legacy-v11',legacyFingerprint:fingerprint,legacyStatus:String(task?.status||'unknown'),
...
export function normalizeLegacyTask(task,existing=null){
  ...
  if(existing?.metadata?.legacyFingerprint===fingerprint)return existing;
  ...
  return {
    ...
    plan:null,metadata:operationalMetadata(task,fingerprint),
...
function collectionFingerprint(rows=[]){return JSON.stringify((Array.isArray(rows)?rows:[]).map(legacyFingerprint))}
...
export function installOfficeV2TaskRuntimeBridge({storage,pollMs=DEFAULT_POLL_MS}={}){
  ...
  window.dispatchEvent(new CustomEvent('ai-office-v2-task-bridge-ready',{detail:{version:OFFICE_V2_TASK_BRIDGE_VERSION}}));
  return true;
}
