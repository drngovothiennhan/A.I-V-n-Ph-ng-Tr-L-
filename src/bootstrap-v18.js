...
function saveTurn(role, text, meta = {}) { const history = loadHistory(); history.push({ role, text: String(text || '').slice(0, 6000), at: new Date().toISOString(), ...meta }); localStorage.setItem(CHAT_KEY, JSON.stringify(history.slice(-MAX_HISTORY))); }
function words(text) { return String(text || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').split(/[^a-z0-9]+/).filter((x) => x.length > 2); }
function classifyIntent(text) {
  const raw = String(text || '').trim();
  const q = raw.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  const taskPatterns = [/^(hay|vui long|giup|lam|tao|soan|viet|xuat|loc|doi chieu|kiem tra|phan tich|trien khai|nang cap|sua|cap nhat|ket noi|gui|lap|chuan bi|thuc hien|thi hanh|tim cho toi)\b/, /\b(tạo|soạn|xuất|lọc|đối chiếu|triển khai|nâng cấp|sửa|thực hiện|thi hành|giao việc|làm giúp|hãy làm)\b/i];
  const questionPatterns = [/\?\s*$/, /^(ai|gi|nao|tai sao|vi sao|khi nao|o dau|bao nhieu|the nao|co phai|giai thich|cho toi biet|hay cho biet)\b/];
...
function localContext(query) { try { if (typeof window.ctx === 'function') return String(window.ctx(query) || '').slice(0, 12000); } catch {} return ''; }
...
async function duckSource(query) {
  try {
    const u = new URL('https://api.duckduckgo.com/'); u.search = new URLSearchParams({ q: query, format: 'json', no_html: '1', no_redirect: '1', skip_disambig: '0' });
    const j = await fetchJson(u.toString()); if (!j?.AbstractText) return [];
...
async function pubmedSources(query) {
  const medical = /\b(bệnh|thuốc|y học|y tế|sức khỏe|triệu chứng|điều trị|chẩn đoán|dược|medicine|health|disease|drug|therapy|diagnosis)\b/i.test(query); if (!medical) return [];
  try {
    const search = new URL('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi'); search.search = new URLSearchParams({ db: 'pubmed', term: query, retmax: '3', sort: 'relevance', retmode: 'json' });
    const s = await fetchJson(search.toString()); const ids = s?.esearchresult?.idlist || []; if (!ids.length) return [];
    const summary = new URL('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi'); summary.search = new URLSearchParams({ db: 'pubmed', id: ids.join(','), retmode: 'json' });
...
function renderAnswer(answer, sources) {
  if (bubble) bubble.textContent = answer; if (!answerBox) return;
  const safe = String(answer).replace(/[&<>]/g, (m) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[m]));
  const refs = sources.slice(0, 8).map((s, i) => { const label = `[${i + 1}] ${String(s.source).replace(/[&<>]/g, '')}`; if (!s.url) return `<span>${label}</span>`; try { const u = new URL(s.url); if (!['http:', 'https:'].includes(u.protocol)) return `<span>${label}</span>`; return `<a href="${u.toString()}" target="_blank" rel="noopener noreferrer">${label}</a>`; } catch { return `<span>${label}</span>`; } }).join(' · ');
  answerBox.innerHTML = `<div class="ai18Answer"><b>Trưởng phòng A.I</b><br>${safe}<div class="ai18Sources">Nguồn đã truy cập: ${refs || 'Drive Brain / nguồn nội bộ đã duyệt'}</div></div>`;
}
function speakThenResume(text) {
  ...
  speaking = true; voice.stopBrowserListening(); window.speechSynthesis.cancel();
  const utterance = new SpeechSynthesisUtterance(clean); utterance.lang = 'vi-VN'; utterance.rate = 1.02;
...
async function handleQuestion(text, { spoken = false } = {}) {
  status.textContent = 'Đang truy cập nguồn & suy luận…'; if (bubble) bubble.textContent = 'Tôi đang truy cập Drive Brain, nguồn học thuật và nguồn ngoài phù hợp…';
  const sources = await gatherSources(text); const answer = await reasonAnswer(text, sources); renderAnswer(answer, sources);
  saveTurn('user', text, { intent: 'question' }); saveTurn('assistant', answer, { intent: 'answer', sourceCount: sources.length });
  status.textContent = continuousVoice ? 'XiaoZhi · hội thoại liên tục' : 'Sẵn sàng'; if (spoken || continuousVoice) speakThenResume(answer); return answer;
}
async function handleTask(text, { spoken = false } = {}) {
  saveTurn('user', text, { intent: 'task' }); if (input) input.value = text;
  let acknowledgement = 'Tôi đã nhận nhiệm vụ và chuyển vào workflow xử lý. Tôi sẽ giữ ngữ cảnh cuộc hội thoại này để các lượt sau có thể tiếp tục công việc.';
  try {
    if (typeof window.submit === 'function') await window.submit();
    const explicitArtifact = /\b(docx|xlsx|pptx|pdf|png|word|excel|powerpoint|file|tệp|bảng tính|trình bày|slide|hình ảnh)\b/i.test(text);
    if (!explicitArtifact) {
      const key = 'ai-office-tasks-v11'; const list = JSON.parse(localStorage.getItem(key) || '[]'); const task = list.find((x) => String(x.originalMessage || '').trim() === text.trim());
      if (task) { task.artifactFormats = []; task.outputMode = 'conversation'; task.requestedArtifact = false; localStorage.setItem(key, JSON.stringify(list)); if (typeof window.render === 'function') window.render(); }
      acknowledgement = 'Tôi đã nhận nhiệm vụ và chuyển vào workflow xử lý. Sản phẩm mặc định sẽ trả ngay trong hội thoại; chỉ tạo DOCX, XLSX, PPTX, PDF hoặc PNG khi bạn yêu cầu rõ.';
    }
  } catch {}
  saveTurn('assistant', acknowledgement, { intent: 'task_ack' }); status.textContent = continuousVoice ? 'XiaoZhi · đang chờ câu tiếp theo' : 'Nhiệm vụ đã tiếp nhận'; if (spoken || continuousVoice) speakThenResume(acknowledgement); return acknowledgement;
}
async function handleMessage(text, options = {}) {
  const value = String(text || '').trim(); if (!value || processing) return; processing = true;
  try { const intent = classifyIntent(value); lastIntent = intent; return intent === 'task' ? await handleTask(value, options) : await handleQuestion(value, options); } finally { processing = false; }
}
voice.addEventListener('state', (event) => { const state = event.detail.state; voiceButton.dataset.state = state; if (state === 'connected') status.textContent = 'XiaoZhi realtime · đã kết nối'; else if (state === 'browser-listening') status.textContent = continuousVoice ? 'XiaoZhi · đang nghe liên tục…' : 'Đang nghe…'; else if (state === 'fallback') status.textContent = 'XiaoZhi Hybrid · Browser Voice + External Brain'; });
voice.addEventListener('partial', (event) => { if (input && !speaking) input.value = event.detail.text || ''; });
voice.addEventListener('transcript', async (event) => { if (Date.now() < ignoreTranscriptUntil || speaking || processing) return; const text = String(event.detail.text || '').trim(); if (!text) return; if (input) input.value = text; if (continuousVoice) voice.stopBrowserListening(); await handleMessage(text, { spoken: true }); });
voice.addEventListener('assistant', (event) => { const text = String(event.detail?.text || event.detail?.message || '').trim(); if (text) { renderAnswer(text, []); saveTurn('assistant', text, { intent: 'xiaozhi_external' }); speakThenResume(text); } });
function toggleContinuousVoice(event) {
  event?.preventDefault?.(); event?.stopImmediatePropagation?.(); continuousVoice = !continuousVoice; localStorage.setItem(VOICE_KEY, continuousVoice ? '1' : '0'); voiceButton.dataset.mode = continuousVoice ? 'continuous' : 'off';
  ...
  if (provider.xiaozhi?.configured) voice.connect(); const started = voice.startBrowserListening(); status.textContent = started ? 'XiaoZhi · đang nghe liên tục…' : 'Hãy cấp quyền microphone để bắt đầu';
}
