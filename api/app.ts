...
function fallbackHtml(message = 'Đang khởi động A.I Văn phòng') {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="5"><title>A.I Văn phòng v1.8</title><style>body{font-family:system-ui;margin:0;display:grid;place-items:center;min-height:100vh;background:#f6f8ff;color:#152449}.box{background:#fff;border:1px solid #e2e8f5;border-radius:18px;padding:28px;max-width:560px;box-shadow:0 14px 40px #263d7717}h1{margin:0 0 10px;font-size:24px}p{color:#66708b}</style></head><body><div class="box"><h1>A.I Văn phòng v1.8</h1><p>${message}</p><p>Hệ thống sẽ tự tải lại.</p></div></body></html>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(SOURCE, {
      headers: { 'user-agent': 'AI-Office-Vercel/1.8' },
      signal: AbortSignal.timeout(8000)
    });
    if (!upstream.ok) return res.status(503).send(fallbackHtml(`Không đọc được giao diện canonical (${upstream.status}).`));
    let html = await upstream.text();
    html = html
      .replace(/A\.I Văn phòng v1\.6/g, 'A.I Văn phòng v1.8')
      .replace(/A\.I Văn phòng v1\.7/g, 'A.I Văn phòng v1.8')
      .replace(/RELEASE 1\.6/g, 'RELEASE 1.8')
      .replace(/RELEASE 1\.7/g, 'RELEASE 1.8')
