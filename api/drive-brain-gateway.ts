import driveBrain from './drive-brain.js';
...
function sameOriginDriveRequest(req) {
...
export default async function handler(req, res) {
  ...
  if (!sameOriginDriveRequest(req)) {
    return json(res, 403, {
      error: 'DRIVE_INTERNAL_SAME_ORIGIN_REQUIRED',
      providerCallMade: false
    });
  }
  return driveBrain(req, res);
}
