...
export const VERSION='3.2.4-neon-direct-grounded';
const DEFAULT_GROUNDED_MODEL='gemini-3.5-flash-lite';
...
function directModel(){return clean(process.env.AI_OFFICE_GEMINI_GROUNDED_MODEL||process.env.AI_OFFICE_GEMINI_MODEL||process.env.GEMINI_MODEL||DEFAULT_GROUNDED_MODEL,120).replace(/^google\//,'')}
...
function normalizeSources(chunks=[]){
  return (Array.isArray(chunks)?chunks:[]).map((chunk,index)=>{
    const url=clean(chunk?.web?.uri||'',1500);
    if(!url)return null;
    return {
      kind:'grounded-web',
...
async function fastGemini(req,res){
  ...
  if(!key)return v31(req,res);
  const started=Date.now();
  const usedModel=directModel();
  ...
  try{
    ...
    if(!response.ok){
      console.warn('research_v32_direct_gemini_rejected',{status:response.status,model:usedModel});
      return v31(req,res);
    }
    const data=await response.json();
    const candidate=data?.candidates?.[0]||{};
    const answer=stripMarkup(candidate?.content?.parts?.map(part=>part?.text||'').join('')||'');
    const sources=normalizeSources(candidate?.groundingMetadata?.groundingChunks||[]);
    if(!answer)return v31(req,res);
    ...
    return json(res,200,{
      ...
      transport:'google-generativelanguage-direct',
      model:usedModel,
      answer:finalAnswer,
      sources,
      grounded:verified,
      groundingMode:'native-google-search-direct',
    ...
    console.warn('research_v32_direct_gemini_error',{name:clean(error?.name||'Error',80),message:clean(error?.message||error,180),model:usedModel});
    return v31(req,res);
  }
}
