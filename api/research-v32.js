...
export const VERSION='3.2.0-gemini-fast-qa';
...
async function fastGemini(req,res){
  ...
  try{
    ...
    if(!response.ok){
      console.warn('research_v32_fast_gemini_rejected',{status:response.status,model:model()});
      return json(res,503,{error:'GEMINI_FAST_REJECTED',status:response.status,provider:'gemini-google-search-fast',model:model(),verificationStatus:'needs_verification',latencyMs:Date.now()-started});
    }
    ...
    if(!answer)return json(res,503,{error:'GEMINI_FAST_EMPTY',provider:'gemini-google-search-fast',model:model(),verificationStatus:'needs_verification',latencyMs:Date.now()-started});
    ...
    console.warn('research_v32_fast_gemini_error',{message:String(error?.message||error).slice(0,160),model:model()});
    return json(res,503,{error:'GEMINI_FAST_UNAVAILABLE',provider:'gemini-google-search-fast',model:model(),verificationStatus:'needs_verification',latencyMs:Date.now()-started});
  }
}
