...
test('workspace model prioritizes recent tasks and exposes safe controls only for latest matching task',()=>{
  const tasks=[
    {id:'old-run',title:'Việc cũ',status:TASK_STATES.RUNNING,updatedAt:'2026-09-12T08:01:00Z',metadata:{progress:30}},
    {id:'new-run',title:'Việc mới',status:TASK_STATES.RUNNING,updatedAt:'2026-09-12T08:03:00Z',metadata:{progress:60,artifactFormats:['docx']}},
    {id:'approval',title:'Chờ duyệt',status:TASK_STATES.WAITING_APPROVAL,updatedAt:'2026-09-12T08:02:00Z',metadata:{progress:96,qaScore:95,needsApproval:true,artifactFormats:['xlsx']}},
  ...
  assert.equal(OFFICE_V2_TASK_WORKSPACE_VERSION,'2.9.0-job-cards');
  assert.equal(model.total,4);assert.equal(model.active,2);assert.equal(model.waitingApproval,1);assert.equal(model.completed,1);
...
test('canonical bootstrap installs task bridge before workspace',()=>{
  const bridge=bootstrap.indexOf("task-runtime-bridge.js?v=280");
  const ui=bootstrap.indexOf("task-workspace.js?v=290");
  assert.ok(bridge>=0&&ui>bridge);
  assert.match(bootstrap,/AIOfficeOfficeV2TaskWorkspaceVersion='2\.9\.0-job-cards'/);
});
